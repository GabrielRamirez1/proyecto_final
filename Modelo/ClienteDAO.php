...
class ClienteDAO
{
    ...
    public function ClienteLogin($usuario, $contrasena)
    {
        $conexion = new ConexionPDO();
        $conexion->Conectar();
        $sql = "SELECT * FROM cliente WHERE c_identificacion = '$usuario' AND c_contrasena = '$contrasena'";
        $resultado = $conexion->CantidadRegistro($sql);
        $conexion->Desconectar();
        return $resultado;
    }

    public function Contarusuario($identificacion)
    {
        $conexion = new ConexionPDO();
        $conexion->Conectar();
        $sql = "SELECT * FROM cliente WHERE c_identificacion = '$identificacion' ";
        $resultado = $conexion->CantidadRegistro($sql);
        $conexion->Desconectar();
        return $resultado;
    }

    public function ConsultarLogin($usu, $contr)
    {
        $conexion = new ConexionPDO();
        $conexion->Conectar();
        $sql = "SELECT * FROM cliente WHERE c_identificacion = '$usu' AND c_contrasena = '$contr'";
